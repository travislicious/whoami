...
function TraitsPage() {
    ...
    function calculateTotal() {
        ...
        setCopyTxt(langQuery === "fr" ? `\nTraits de *${person?.name}.*\n\n *Confiance:* ${person?.traits[0].percent}%\n *Sourire*: ${person?.traits[1].percent}%\n *Mignonnerie*: ${person?.traits[2].percent}%\n *Amour*: ${person?.traits[3].percent}%\n *Gentillesse*: ${person?.traits[4].percent}%\n *Colère*: ${person?.traits[5].percent}%\n\n *Total*: ${totalPercent}%\n\n Allez sur \`\`\`whoami-web.vercel.app?lang=fr\`\`\` pour voir le vôtre.\n`: `\n*${person?.name}'s* Traits.\n\n *Confidence*: ${person?.traits[0].percent}%\n *Smile*: ${person?.traits[1].percent}%\n *Cuteness*: ${person?.traits[2].percent}%\n *Love*: ${person?.traits[3].percent}\n *Kindness*: ${person?.traits[4].percent}%\n *Anger*: ${person?.traits[5].percent}%\n\n *Total:* ${totalPercent}%\n\n Go on \`\`\`whoami-web.vercel.app?lang=en\`\`\` to see your own.\n`)

        setStatutTxt(langQuery === "fr" ? `whatsapp://send?text=%0ATraits de *${person?.name}.*%0A%0A *Confiance:* ${person?.traits[0].percent}% %0A *Sourire*: ${person?.traits[1].percent}% %0A *Mignonnerie*: ${person?.traits[2].percent}% %0A *Amour*: ${person?.traits[3].percent}% %0A *Gentillesse*: ${person?.traits[4].percent}% %0A *Colère*: ${person?.traits[5].percent}% %0A%0A *Total*: ${totalPercent}% %0A%0A Allez sur \`\`\`whoami-web.vercel.app?lang=fr\`\`\` pour voir le vôtre.%0A`: `whatsapp://send?text=%0A*${person?.name}'s* Traits.*%0A%0A *Confidence*: ${person?.traits[0].percent}% %0A *Smile*: ${person?.traits[1].percent}% %0A *Cuteness*: ${person?.traits[2].percent}% %0A*Love*: ${person?.traits[3].percent}% %0A *Kindness*: ${person?.traits[4].percent}% %0A *Anger*: ${person?.traits[5].percent}% %0A%0A *Total:* ${totalPercent}% %0A%0A Go on \`\`\`whoami-web.vercel.app?lang=en\`\`\` to see your own.%0A`)
    }
    ...
    return (
        <main className="h-screen w-screen flex flex-col justify-center bg-black items-center text-white">
            ...
            <div className="mt-6 border h-[30rem] flex flex-col items-center rounded-lg gap-2 border-neutral-800 mb-6 w-[28rem] max-sm:w-96">
